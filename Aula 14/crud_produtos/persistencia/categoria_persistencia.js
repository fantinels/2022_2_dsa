...
async function listar() {
    ...
    const res = await cliente.query('SELECT * FROM categorias ORDER BY id');
...
async function buscarPorId(id) {
    ...
    const res = await cliente.query('SELECT * FROM categorias WHERE id = $1', 
        [id]);
...
async function deletar(id) {
    ...
    const res = await cliente.query('DELETE FROM categorias WHERE id = $1 RETURNING *', 
        [id]);
